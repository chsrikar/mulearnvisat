import { motion } from 'framer-motion'

function ThemeToggle({ darkMode, setDarkMode, className = '' }) {
    return (
        <button
            type="button"
            id="theme-toggle"
            aria-label="Toggle dark mode"
            className={`rounded-full transition-all duration-300 active:scale-95 cursor-pointer border-none flex items-center justify-center ${darkMode ? 'bg-surface-800 text-white' : 'bg-surface-100 text-surface-900'
                } ${className}`}
            onClick={() => setDarkMode(!darkMode)}
        >
            <svg
                xmlns="http://www.w3.org/2000/svg"
                aria-hidden="true"
                fill="currentColor"
                strokeLinecap="round"
                viewBox="0 0 32 32"
                className="w-full h-full p-2"
            >
                <clipPath id="theme-toggle-clip">
                    <motion.path
                        animate={{ y: darkMode ? 10 : 0, x: darkMode ? -12 : 0 }}
                        transition={{ ease: 'easeInOut', duration: 0.35 }}
                        d="M0-5h30a1 1 0 0 0 9 13v24H0Z"
                    />
                </clipPath>
                <g clipPath="url(#theme-toggle-clip)">
                    <motion.circle
                        animate={{ r: darkMode ? 10 : 8 }}
                        transition={{ ease: 'easeInOut', duration: 0.35 }}
                        cx="16"
                        cy="16"
                    />
                    <motion.g
                        animate={{
                            rotate: darkMode ? -100 : 0,
                            scale: darkMode ? 0.5 : 1,
                            opacity: darkMode ? 0 : 1,
                        }}
                        transition={{ ease: 'easeInOut', duration: 0.35 }}
                        stroke="currentColor"
                        strokeWidth="1.5"
                    >
                        <path d="M16 5.5v-4" />
                        <path d="M16 30.5v-4" />
                        <path d="M1.5 16h4" />
                        <path d="M26.5 16h4" />
                        <path d="m23.4 8.6 2.8-2.8" />
                        <path d="m5.7 26.3 2.9-2.9" />
                        <path d="m5.8 5.8 2.8 2.8" />
                        <path d="m23.4 23.4 2.9 2.9" />
                    </motion.g>
                </g>
            </svg>
        </button>
    )
}

export default ThemeToggle
